...
public class ConferenceMemberView extends RelativeLayout {
    ...
    private TextView nameView, text_net_state;
    ...
    public void setNetState(String content) {
        if (null != text_net_state)
            text_net_state.setText(content);
    }

    public void setVisibleNetStateUi(int visible) {
        if (null != text_net_state)
            text_net_state.setVisibility(visible);
    }
    ...
    public void setUsername(String username) {
        ...
        if (activity.getIntent().getIntExtra(KEY_CALL_TYPE, -1) == 1) {
            avatarView.setImageResource(R.drawable.em_call_mic_on);
        }else
        avatarView.setImageResource(R.drawable.em_call_video_default);
        nameView.setText(username);
    }

    public void setDefaultBackgroundView(int id) {
        avatarView.setVisibility(View.VISIBLE);
