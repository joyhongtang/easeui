...
import com.hyphenate.chatuidemo.widget.DragSurfaceView;
...
public class VideoCallActivity extends CallActivity implements OnClickListener, DragSurfaceView.DragSurfaceViewClickLis {

    /**
     * 是否打开视频通话实时参数显示
     */
    private boolean openConferenceOption = false;
    ...
    protected DragSurfaceView localSurface;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // local surfaceview
        localSurface = (DragSurfaceView) findViewById(R.id.local_surface);
        localSurface.setDragSurfaceViewClickLis(this::onDragSurfaceClick);
        localSurface.setOnClickListener(this);
        localSurface.setZOrderMediaOverlay(true);
        localSurface.setZOrderOnTop(true);
        localSurface.setScaleMode(EMCallViewScaleModeAspectFill);
        localSurface.getRenderer().setScalingType(RendererCommon.ScalingType.SCALE_ASPECT_BALANCED);
        localSurface.getHolder().addCallback(new SurfaceHolder.Callback() {
            @Override
            public void surfaceCreated(SurfaceHolder holder) {
                Log.i("localSurface","surfaceCreated");
            }

            @Override
            public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
                Log.i("localSurface","surfaceChanged");
            }

            @Override
            public void surfaceDestroyed(SurfaceHolder holder) {
                Log.i("localSurface","surfaceDestroyed");
            }
        });
        ...
        oppositeSurface.setScaleMode(EMCallViewScaleModeAspectFill);
        oppositeSurface.getRenderer().setScalingType(RendererCommon.ScalingType.SCALE_ASPECT_BALANCED);
...
//                callHelper.startVideoRecord(PathUtil.getInstance().getVideoPath().getAbsolutePath());
    ...
    protected void onDestroy() {
        ...
        localSurface.getRenderer().release();
        oppositeSurface.getRenderer().release();
        ...
        new Thread(new Runnable() {
            public void run() {
                while (monitor) {
                    runOnUiThread(new Runnable() {
                        public void run() {
                            if (openConferenceOption) {
                                StringBuilder config = new StringBuilder("WidthxHeight：" + callHelper.getVideoWidth() + "x" + callHelper.getVideoHeight()
                                        ...
                                        + "\n" + recordString);
                                monitorTextView.setText(config.toString());

                                String incomingORCall = getResources().getString(EMClient.getInstance().callManager().isDirectCall()
                                        ? R.string.direct_call : R.string.relay_call);
                                if (openConferenceOption)
                                ((TextView) findViewById(R.id.tv_is_p2p)).setText(incomingORCall);
                                config.append(incomingORCall);
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        localSurface.getRenderer().clearImage();
        oppositeSurface.getRenderer().clearImage();
        super.onConfigurationChanged(newConfig);
    }

    @Override
    public void onDragSurfaceClick(View view) {
        changeCallView();
    }
}
