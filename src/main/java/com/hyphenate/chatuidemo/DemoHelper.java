...
//        }
        ...
        EMClient.getInstance().callManager().getCallOptions().setVideoResolution(1920, 1080);
        EMClient.getInstance().callManager().getCallOptions().setMinVideoKbps(2000);
...
//                return intent;
    ...
    protected void setGlobalListeners() {
        ...
        EMClient.getInstance().conferenceManager().addConferenceListener(new EMConferenceListener() {
            ...
            public void onMute(String s, String s1) {
                try {
                    ConferenceActivity conferenceActivity = (ConferenceActivity) ActivityPageManager.getInstance().findActivity("ConferenceActivity");
                    conferenceActivity.muteSound();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            public void onUnMute(String s, String s1) {
                try {
                    ConferenceActivity conferenceActivity = (ConferenceActivity) ActivityPageManager.getInstance().findActivity("ConferenceActivity");
                    conferenceActivity.unmuteSound();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            @Override
            public void onStreamStateUpdated(String s, StreamState streamState) {
    ...
    public void goConference(String confId, String password, String extension) {
        try {
            ...
            final String inviter = jsonObj.optString(Constant.EXTRA_CONFERENCE_INVITER);
            final String groupId = jsonObj.optString(Constant.EXTRA_CONFERENCE_GROUP_ID);
            final int callType = jsonObj.optInt(KEY_CALL_TYPE);
            final int singlecall = jsonObj.optInt(KEY_CALL_SINGLE);
            if (isDuringMediaCommunication(inviter)) {
                return;
            }
                ...
                public void onSuccess(Object o) {
                    PermissionUtils.permission(PermissionConstants.CAMERA, PermissionConstants.MICROPHONE).callback(new PermissionUtils.FullCallback() {
                        ...
                        public void onGranted(List<String> permissionsGranted) {
                            ...
                            boolean lockScreen = keyguardManager.inKeyguardRestrictedInputMode();
                            if (lockScreen && ConferenceConfig.machineType == ConferenceConfig.ConfigMachineType.PHONE) {
                                ConferenceConfig.setLockScreenStatus(appContext, confId, password, inviter, groupId, callType, singlecall);
                            } else {
                                ConferenceActivity.receiveConferenceCall(appContext, confId, password, inviter, groupId, callType, singlecall);
                            }
                            Log.i(TAG, "start" + String.valueOf(o));
                        }
...
//        LiveActivity.watch(appContext, confId, password, inviter);
    ...
    private boolean isDuringMediaCommunication(String inviter) {
        try {
    ...
    /**
     * A CALL TO B ,BUT B IS IN CONFERENCE
     *
     * @param inviter
     */
    private void createRejectMsg(String inviter) {
        final EMMessage message = EMMessage.createTxtSendMessage("You have an incoming call", inviter);
        // set the user-defined extension field
        message.setAttribute("reject_call_busy", true);
...
//        appContext.startActivity(intent);
    ...
    public HashMap<String,String> destoryConferenceList = new HashMap<>();
    /**
     * Global listener
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    String destroy_conference_id = message.getStringAttribute("destroy_conference_id","");
                    if(destoryConferenceList.containsKey(destroy_conference_id)){
                        return;
                    }
                    if(!TextUtils.isEmpty(destroy_conference_id)){
                        destoryConferenceList.put(destroy_conference_id,destroy_conference_id);
                    }
                    //是否拒绝语音通话
                    boolean reject_call_info = message.getBooleanAttribute("reject_call", false);
                    if (reject_call_info) {
                        try {
                            ...
                            ConferenceActivity conferenceActivity = (ConferenceActivity) ActivityPageManager.getInstance().findActivity("ConferenceActivity");
                            conferenceActivity.exitConference();
                            showToast("The other party may not be online");
                            return;
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    //拨打的用户是否正在忙线中
                    boolean reject_call_busy_info = message.getBooleanAttribute("reject_call_busy", false);
                    if (reject_call_busy_info) {
                        try {
                            ConferenceActivity conferenceActivity = (ConferenceActivity) ActivityPageManager.getInstance().findActivity("ConferenceActivity");
                            if(!conferenceActivity.startConference) {
                                conferenceActivity.exitConference();
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        ToastUtils.showLong("This friend may be not online");
                        return;
                    }
                    // 判断一下是否是会议邀请
