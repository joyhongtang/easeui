...
abstract class CallBaseHttpObserver<T> : Observer<Base<T>> {
    ...
    protected constructor(context: Context?) {
        context?.apply {
            ...
            mDialog = Dialog(context, R.style.CallLoadingDialog)
    ...
    override fun onError(e: Throwable) {
        ...
        /*if(!TextUtils.isEmpty(e.message)){
            if(e.message!!.contains("java.lang")){
                return
            }
            if(e.message!!.contains("bsimb.cn") && e.message!!.contains("failed to connect")){
                ToastUtils.showLong("failed to connect server")
                return
            }
        }
    ...
    open fun onFail(status: Int) {
        when (status) {
            ...
            118 -> ToastUtils.showShort("$status, ${Utils.getApp().getString(R.string.e118)}")
            ...
            142 -> ToastUtils.showShort("$status, ${Utils.getApp().getString(R.string.e121)}")
